...
winning_prod <- primes[winning_prod] %>% matrix(nrow = 8, ncol = 3, byrow = TRUE) 
...
seventh_turn_wins <- nrow(x[seventh>0&sixth==0&fifth==0,])/factorial(2)
...
ninth_turn_wins <- nrow(x[ninth>0&eigth==0&seventh==0&sixth==0&fifth==0,])

total_wins <- nrow(x[ninth>0,])
total_draws <- nrow(x[ninth==0,])

draws <- nrow(x[ninth==0&eigth==0&seventh==0&sixth==0&fifth==0,])

#rm(winning_seqs,winning_prod,x,fifth,sixth,seventh,eigth,ninth,primes,first,second,mod)
